...
- **Phase 7.2: 到達可能性ギャップの修正と拡張比較**:
  ...
  ユーザー待ち）
- **Phase 7.3: 焦点を絞った比較調査(コード変更なし)**:
  [docs/P7_3_STATUS.md](docs/P7_3_STATUS.md)
  （P7.2で残った43件をMath-Graph自身のスキーマ列だけで分類——`kind`列
  (inst/instance)は全38.8万件の分布を見ると単なる略語/正式名の混在と
  判明し却下、`via_proj`列は両群とも100%falseで無情報と判明し却下、
  `edge_type`列は決定的: 一致した宣言の辺は`proof`型を含むが、未解決の
  42件のOrderDual.*は`proof`型が0件で`sig`/`def`型のみ——さらに内容を
  見るとその`def`辺がMathlibの実際の`extends`階層(Monoid⇐Semigroup+
  MulOneClassなど)を寸分違わず再現していると確認。43件中41件を「型クラス
  階層の合成ノード」として再分類(名前の類似ではなくスキーマ+内容証拠)。
  一致した宣言どうしの依存本数を比較すると両者は同程度(優劣なし)——
  Math-Graphは「文書レベルで一致する宣言には追加のrecallをもたらさない」
  が「Mathesisの宣言モデルが原理的に持てない型クラス階層構造の情報は
  もたらす」と結論。役割は「信頼グラフへの正規入力」ではなく「比較検証
  レイヤー」+「型クラス階層に限った発見的補助」と結論——広範化するかは
  引き続きユーザー判断）
- **Phase 7.4: 本番向け・境界を絞ったMath-Graph発見レイヤー**:
  [docs/P7_4_STATUS.md](docs/P7_4_STATUS.md)
  （`external_literal_dependency`/`external_typeclass_hierarchy`という
  別軸を追加(`epistemic_state: extracted`・`traversal_policy:
  visible_only`は不変)。P7.3の「41件+2件」という概算を実装のために
  厳密な規則で再導出したところ「42件+1件」だったと判明・修正して記録。
  安全な62件だけを2つの隔離DBへ取り込み、既存の`dependencies.json`とは
  完全に独立した新しい比較/発見モードパネルをブラウザで実装・検証
  （既定は非表示、トグルで48件の外部辺を表示、それぞれに出典・
  ライセンス・「Mathesisが独自検証していない」旨を明記）。評価の結果:
  文字通り一致する宣言どうしでは新規発見0件(100%重複)——価値はすべて
  「Mathesis自身の宣言モデルには存在しえない」42件の型クラス階層側に
  ある。P6.3以来の「生成物の陳腐化」バグ再発防止のため、ハッシュが
  自己無矛盾でもフィールド形状の変化だけを検出する回帰テストを追加。
  本番`scratch/provenance.db`・検索・既定の系譜ビューは無変更のまま
  検証済み）
